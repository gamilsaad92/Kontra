...
export default function Applications(){
  ...
  async function load(){
    try {
      ...
      // If your API returns {data: [...]}, adapt: setItems(data.data)
  ...
  async function create(e:React.FormEvent<HTMLFormElement>){
    ...
    const payload = {
      borrower_name: String(fd.get('borrower_name')||'').trim(),
      amount: Number(fd.get('amount')||0),
      property_address: String(fd.get('property_address')||'').trim()
    }
    if (!payload.borrower_name || !Number.isFinite(payload.amount)) {
      setErr('Enter a valid borrower name and amount')
      return
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Table>
        ...
        {items.map(x => (
          <TRow key={x.id}>
            <TCell span={3}>{x.borrower_name}</TCell>
            <TCell span={2}>${x.amount.toLocaleString()}</TCell>
            <TCell span={5}>{x.property_address || '—'}</TCell>
            <TCell span={2}>{x.created_at?.slice(0,10) ?? '—'}</TCell>
          </TRow>
        ))}
      </Table>
    </div>
